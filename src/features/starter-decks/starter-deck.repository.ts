...
function starterContentType(starter: StarterDeck) {
  const types = new Set(starter.file.deck.entries.map((entry) => entry.type));
  return types.size === 1 ? starter.file.deck.entries[0]?.type : undefined;
}
...
export async function copyStarterEntry(database: Database, starter: StarterDeck, entry: StarterEntry, destinationDeckId: number) {
  ...
  if (destination.sourceLanguage !== starter.file.deck.sourceLanguage || destination.targetLanguage !== starter.file.deck.targetLanguage || destination.contentType !== entry.type) throw new Error('INCOMPATIBLE_DECK');
...
export async function copyStarterEntries(database: Database, starter: StarterDeck, selected: StarterEntry[], destinationDeckId: number) {
  ...
  return database.transaction(async (transaction) => {
    ...
    if (!destination) throw new Error('DESTINATION_NOT_FOUND');
    if (destination.isArchived) throw new Error('DESTINATION_ARCHIVED');
    if (destination.sourceLanguage !== starter.file.deck.sourceLanguage || destination.targetLanguage !== starter.file.deck.targetLanguage || selected.some((entry) => entry.type !== destination.contentType)) throw new Error('INCOMPATIBLE_DECK');
...
export async function installStarterDeck(database: Database, starter: StarterDeck) {
  return database.transaction(async (transaction) => {
    ...
    const contentType = input.contentType ?? starterContentType(starter);
    if (!contentType) throw new Error('MIXED_STARTER_DECK');
    const [deck] = await transaction.insert(decks).values({ name: input.name, description: input.description?.trim() || null, sourceLanguage: input.sourceLanguage, targetLanguage: input.targetLanguage, color: input.color ?? '#355A46', contentType, createdAt: now, updatedAt: now }).returning();
