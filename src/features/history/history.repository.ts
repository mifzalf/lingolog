...
export type HistorySession = { id: number; mode: 'flashcard' | 'dictation' | 'matchup' | 'delayed_recall' | 'word_wheel' | 'mixed'; startedAt: Date; completedAt: Date | null; totalItems: number; correctItems: number; durationMs: number; answered: number; corrected: number; deckNames: string[]; configJson: string | null };
