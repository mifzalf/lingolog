...
import { MasteryPicker } from '../../../src/features/practice/MasteryPicker';
...
export default function DeckScreen() {
  ...
  const [search, setSearch] = useState(''); const [filters, setFilters] = useState<LibraryFilterState>({ sort: 'alphabetical' }); const [filterOpen, setFilterOpen] = useState(false); const [speechSettings, setSpeechSettings] = useState(false);
  const [masteryItem, setMasteryItem] = useState<EntryListItem>(); const [masterySaving, setMasterySaving] = useState(false);
  ...
  async function changeMastery(grade: MasteryGrade) { if (!masteryItem) return; try { setMasterySaving(true); await setManualMasteryGrade(database, masteryItem.id, deckId, grade); setMasteryItem(undefined); await load(); } catch (cause) { console.error(cause); } finally { setMasterySaving(false); } }
  const filtering = Boolean(search.trim()) || activeFilterCount(filters) > 0;

  return <PaperScreen keyboardShouldPersistTaps="handled">
    <View style={styles.topbar}><IconButton name="arrow-back" label="Kembali" onPress={() => router.back()} /><View style={styles.heading}><Text style={styles.title} numberOfLines={1}>{deck?.name ?? 'Deck'}</Text>{deck ? <Text style={styles.pair}>{getLanguage(deck.sourceLanguage).short} → {getLanguage(deck.targetLanguage).short}</Text> : null}</View><View style={styles.headerActions}><IconButton name="volume-medium-outline" label="Pengaturan suara" onPress={() => setSpeechSettings(true)} /><IconButton name="settings-outline" label="Pengaturan deck" onPress={() => router.push(`/decks/${deckId}/edit`)} /></View></View>
    ...
    {items.map((item) => <Pressable accessibilityRole="button" accessibilityLabel={`Buka ${item.sourceText}`} key={item.id} onPress={() => router.push(`/decks/${deckId}/entries/${item.id}`)} style={({ pressed }) => [styles.entry, pressed && { opacity: 0.68 }]}>
      <View style={styles.entryText}><View style={styles.entryTop}><Text style={styles.source}>{item.sourceText}</Text><Pressable hitSlop={10} accessibilityRole="checkbox" accessibilityState={{ checked: item.isFavorite }} accessibilityLabel={item.isFavorite ? 'Hapus dari favorit' : 'Tambah ke favorit'} onPress={(event) => { event.stopPropagation(); void favorite(item); }}><Ionicons name={item.isFavorite ? 'star' : 'star-outline'} size={20} color={item.isFavorite ? colors.highlight : colors.inkFaint} /></Pressable></View><Text style={styles.translation}>{item.translatedText}</Text><View style={styles.tags}><Pill tone="plain">{typeLabels[item.type]}</Pill><Pressable accessibilityRole="button" accessibilityLabel={`Ubah mastery ${item.sourceText}, saat ini ${gradeLabels[item.grade] ?? 'Baru'}`} onPress={(event) => { event.stopPropagation(); setMasteryItem(item); }} style={[styles.masteryChip, item.grade === 3 && styles.masteryStrong]}><Ionicons name={item.grade === 3 ? 'ribbon' : item.grade === 2 ? 'leaf-outline' : item.grade === 1 ? 'pencil-outline' : 'ellipse-outline'} size={12} color={item.grade === 3 ? colors.primaryInk : colors.primary} /><Text style={[styles.masteryText, item.grade === 3 && styles.masteryStrongText]}>{gradeLabels[item.grade] ?? 'Baru'}</Text><Ionicons name="chevron-down" size={11} color={item.grade === 3 ? colors.primaryInk : colors.primary} /></Pressable>{item.tagNames.slice(0, 2).map((tag) => <Pill key={tag}>{tag}</Pill>)}{item.tagNames.length > 2 ? <Pill tone="plain">+{item.tagNames.length - 2}</Pill> : null}</View></View><SpeakButton compact text={item.sourceText} language={deck?.sourceLanguage ?? ''} speechKey={`list-${item.id}`} label={item.sourceText} /><Ionicons name="chevron-forward" size={18} color={colors.inkMuted} />
    </Pressable>)}
    <MasteryPicker visible={Boolean(masteryItem)} entryText={masteryItem?.sourceText} currentGrade={(masteryItem?.grade ?? 0) as MasteryGrade} saving={masterySaving} onSelect={(grade) => void changeMastery(grade)} onClose={() => setMasteryItem(undefined)} />
    {deck ? <SpeechSettings visible={speechSettings} languages={[getLanguage(deck.sourceLanguage), getLanguage(deck.targetLanguage)]} onClose={() => setSpeechSettings(false)} /> : null}
  </PaperScreen>;
}

const createStyles = (colors: ThemeColors) => StyleSheet.create({
  topbar: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 22 }, headerActions: { flexDirection: 'row', gap: 6 }, heading: { flex: 1 }, title: { color: colors.ink, fontSize: 23, fontWeight: '900' }, pair: { color: colors.primary, fontSize: 12, fontWeight: '900', marginTop: 3 },
  ...
  entry: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: colors.rule }, masteryChip: { minHeight: 27, flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 8, borderRadius: radius.sm, borderWidth: 1, borderColor: colors.primary, backgroundColor: colors.primarySoft }, masteryStrong: { backgroundColor: colors.primary }, masteryText: { color: colors.primary, fontSize: 10, fontWeight: '900' }, masteryStrongText: { color: colors.primaryInk }, entryText: { flex: 1 }, entryTop: { flexDirection: 'row', alignItems: 'center', gap: 10 }, source: { flex: 1, color: colors.ink, fontSize: 17, lineHeight: 22, fontWeight: '900' }, translation: { color: colors.inkMuted, fontSize: 13, lineHeight: 19, marginTop: 3 }, tags: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 9 },
});
