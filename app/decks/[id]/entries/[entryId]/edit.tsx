...
export default function EditEntryScreen() {
  ...
  async function persist(input: EntryInput) { try { setSaving(true); await updateEntry(database, entryId, input); backOrReplace(`/decks/${deckId}/entries/${entryId}`); } catch (error) { console.error(error); showDialog({ title: 'Perubahan belum tersimpan', message: 'Coba simpan kembali. Data yang kamu isi tetap ada.', icon: 'cloud-offline-outline' }); setSaving(false); } }
  ...
  function remove() { showDialog({ title: 'Hapus entri?', message: `“${entry?.sourceText}” akan dihapus beserta statistiknya.`, icon: 'trash-outline', dismissible: false, actions: [{ label: 'Hapus entri', tone: 'danger', onPress: async () => { await deleteEntry(database, entryId); backOrReplace(`/decks/${deckId}`); } }, { label: 'Pertahankan entri', tone: 'neutral' }] }); }
  ...
  return <EntryForm title="Ubah entri" deckId={deckId} sourceName={getLanguage(deck.sourceLanguage).name} targetName={getLanguage(deck.targetLanguage).name} contentType={deck.contentType ?? entry.type} initial={{ type: entry.type, sourceText: entry.sourceText, translatedText: entry.translatedText, notes: entry.notes ?? '', exampleText: entry.exampleText ?? '', exampleTranslation: entry.exampleTranslation ?? '', isFavorite: entry.isFavorite, tags: entry.tags }} saving={saving} onCancel={() => backOrReplace(`/decks/${deckId}/entries/${entryId}`)} onMore={remove} onSave={save} />;
}
